          ...
          <div className="space-y-3 border-t border-stone-200 pt-3">
            ...
            <div className="space-y-3">
              {providers.map((provider, index) => {
                ...
                const providerRef = String(provider.provider_ref || `${type}#${index + 1}`);
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      ...
                      {type === "yyds_mail" ? (
                        <>
                          ...
                          {learningMode ? (
                            <div className="space-y-2">
                              <label className="text-sm text-stone-700">探索比例</label>
                              <Input
                                value={String(provider.domain_explore_rate ?? 0.12)}
                                onChange={(event) => updateProvider(index, { domain_explore_rate: Number(event.target.value) || 0 })}
                                placeholder="0.12"
        ...
        <div className="mt-4 flex min-h-0 flex-1 flex-col space-y-3 overflow-hidden border-t border-stone-200 pt-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-amber-700">遇到 `registration_disallowed` 这类 400 错误时，YYDS 学习模式会自动标记当前域名不可用并切换新邮箱重试；非学习模式通常需要手动更换域名邮箱。</p>
              </div>
