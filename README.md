...
- 支持生成可编辑 PPT 文件
- 支持生成可编辑 PSD 文件
- 调试页支持多模态图片输入，并可传递 `reasoning_effort` / `thinking_effort` 思考强度参数
...
- 注册机页面支持配置账号与额度自动刷新间隔，并显示上次/下次运行时间
- 轮询可用账号执行图片生成与图片编辑
...
- 注册流程已对齐当前官方浏览器链路：邮箱阶段会先走 `authorize` + `authorize/continue`，验证码通过后的 `create_account` 会使用当前官方 Sentinel SDK 生成 `OpenAI-Sentinel-Token` 与 `OpenAI-Sentinel-SO-Token` 两个请求头；QuickJS 运行时会补齐官方 SDK 当前依赖的 DOM / Storage / Navigator / timer 能力，并对 `turnstile.required`、`so.required` 做强校验，遇到 Base64 包装的 Turnstile 运行时错误、缺失 SO 或无效 Sentinel 片段时会立即中断，不再把错误值拼进组合 token 后继续请求；若官方在建号成功后先跳转到 `create-account-enroll-passkey` 引导页，后端会自动调用官方 `passkey enrollment skip` 接口继续换取 OAuth code；密码登录也会复用同一套 SDK 生成逻辑，日志只记录 `p/t/c/so` 长度、required 模式和运行模式，不打印明文
